...
class ChatBrowser {
  ...
  Future<List<Chat>> _loadChats({bool includeEmpty = false}) async {
    ...
    try {
          ...
          if (dbFile.existsSync()) {
            try {
              ...
              for (final row in result) {
                ...
                if (chat != null) {
                  if (Chat.isValidChat(chat) || includeEmpty) {
                    chats.add(chat);
                  } else {
      ...
      if (skippedChats.isNotEmpty) {
        print('Skipped ${skippedChats.length} empty chats (no messages)');
      }
  ...
  Future<List<Chat>> loadAllChats({bool includeEmpty = false}) async {
    final allChats = await _loadChats(includeEmpty: includeEmpty);
    return allChats;
  }
